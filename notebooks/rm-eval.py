...
from nyx.constants import (COMMON_OUTPUT_PATHS, METRICS_PATH,
                           RL_PEFT_ADAPTER_PATH, RM_PEFT_ADAPTER_PATH,
                           RM_PEFT_MERGED_MODEL_PATH, SFT_DATA_OUTPUT_PATH,
                           SFT_PEFT_ADAPTER_PATH)
from nyx.data_generation.prompts.model_specific_tokens import (
    QWEN_BOS_ASSISTANT, QWEN_BOS_USER)
from nyx.evaluation import quantitative_comparison

parser = argparse.ArgumentParser()
parser.add_argument(
    "rm_run_id", type=str, help="RM runId to evaluate the RL generations with."
)
parser.add_argument("batch_size", type=int, help="batch size for evaluation")
parser.add_argument(
    "--use_qwen", action="store_true", help="Evaluate qwen-7b if true otherwise phi-1-5"
)
...
CHOSEN_MODEL = (
    "unsloth/Qwen2-7B-Instruct-bnb-4bit" if USE_QWEN is True else "microsoft/phi-1_5"
)
DEVICE = 'cuda'
...
RM_PEFT_ADAPTER_PATH = RM_PEFT_ADAPTER_PATH.format(COMMON_OUTPUT_PATHS=rm_common_path)
...
if USE_QWEN is True:
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        SFT_PEFT_ADAPTER_PATH, padding_side='left'
    )
...
human_baseline_summary_prompts = [
    prompt.split('Summary:')[0] + f'Summary: {summary}'
    for summary, prompt in zip(human_baseline_answer, peft_checkpoint_generation)
]
...
rm_model = AutoModelForSequenceClassification.from_pretrained(
    RM_PEFT_MERGED_MODEL_PATH, device_map="auto"
)
    ...
    for i in range(0, len(dataset_dict['rm_test']), batch_size):
        with torch.no_grad():
            reward_logits = rm_model_to_evaluate(
                **tokenizer(
                    dataset_dict['rm_test'][col][i : i + batch_size],
                    return_tensors="pt",
                    padding=True,
                    truncation=True,
                ).to(DEVICE)
...
stats_to_log['human_baseline_reward_mean'] = np.round(
    np.mean(human_baseline_rewards), 2
)
...
print(
    np.round(np.mean(ppo_rewards), 2),
    np.round(np.mean(sft_rewards), 2),
    np.round(np.mean(human_baseline_rewards), 2),
)
