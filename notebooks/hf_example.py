...
prompts = [
    ...
    "Who is Yann Le Cun?",
    "How far is the moon?",
    "Who is the host of the tonight show?",
    "What year was Obama born?",
    "When did we land on the moon?",
    "How far is Jupyter?",
    "What is the diameter of the sun?",
    "What can you do to improve your table tennis game?",
    "Who is the all time best player in chess?",
]
...
indices = random.sample(range(0, 16), 8)
f_at_indices = operator.itemgetter(*indices)

# print(indices)
chosen_prompts = list(f_at_indices(prompts))
formatted_prompts = [
    chosen_prompts[i : i + batch_size]
    for i in range(0, len(chosen_prompts), batch_size)
]
...
print(f'from device: {distributed_state.device}', completions_gather, '\n')
distributed_state.print(completions_gather)
# Drop duplicates produced by apply_padding in split_between_processes
completions = completions_gather[: len(chosen_prompts)]
print(f'from device: {distributed_state.device}', completions, '\n')
